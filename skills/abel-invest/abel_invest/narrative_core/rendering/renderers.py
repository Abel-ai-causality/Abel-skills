...
def build_agent_context(
    *,
    ...
    sample_strategies = render_agent_context_sample_strategies(session)
    return f"""# Agent Context Pack

generated by Abel Invest alpha-search narrative layer

{sample_strategies}## Evidence Frontier
...
def render_agent_context_sample_strategies(session: Path) -> str:
    samples = load_available_sample_strategies(session)
    if not samples:
        return ""
    sample_lines = []
    for sample in samples:
        context_path = sample["context_path"].relative_to(session).as_posix()
        source_path = sample["source_dir"].relative_to(session).as_posix()
        sample_lines.append(
            f'- sample {sample["rank"]}: context `{context_path}`; source `{source_path}/`'
        )
    sample_list = "\n".join(sample_lines)
    return f"""## Optional Sample Strategies

{sample_list}

Read each sample's context, source, and recorded exploration history before
broader exploration. Extract reusable lessons from its mechanism, input nodes,
tested choices, and unresolved assumptions. Samples are reference context, not
ready current-session candidates: do not directly reuse their strategy or
source in a current branch, or revalidate them unchanged. Use those lessons
with the current goal, frontier, and data to form new hypotheses and strategy
expressions, then validate them through the normal loop.
