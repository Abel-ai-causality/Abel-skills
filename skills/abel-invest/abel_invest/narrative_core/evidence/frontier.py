...
- explicit_control_or_ablation_evidence: `{concentration.get("target_control_evidence", 0)}`
- comparable_explicit_controls_or_ablations: `{concentration.get("comparable_controls", 0)}`
...
- explicit_control_or_ablation_evidence_count: `{exploration.get("control_evidence_count", 0)}`
...
Path and input coverage mean the evidence is organized; they do not prove
exhaustion. A strong lead is evidence for the ledger, not a mandate for local
scratch search.
...
- explicit_control_or_ablation_evidence: `{comparable.get("target_control_evidence", 0)}`
"""
...
def render_session_frontier_summary(frontier: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- comparable_explicit_control_or_ablation_evidence: `{comparable.get('target_control_evidence', 0)}`",
            ...
            "- search_boundary: `coverage is audit organization, not exhaustion`",
