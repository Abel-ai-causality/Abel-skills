...
def build_context_guide_markdown(
    *,
    ...
    lines = [
        ...
        "- use scout as optional scratch research, not a required branch phase; a first-look scout often fits about 120 seconds and helps choose a direction before broad recorded work",
        "- make scout scripts bounded and resumable: run `--dry-run` first with a free-form family budget declaration, treat declared budget seconds as advisory search-shape disclosure rather than trusted timing or an execution gate, stream completed candidates to `scratch/*.results.jsonl`, keep `scratch/*.state.json` and `scratch/*.summary.json` current, and use `--resume` after interruption instead of restarting",
        ...
        "- `ScoutRun` tracks cumulative scout runtime as an execution guardrail; when scout has enough directional evidence or runtime stops it, turn current artifacts into a concrete branch, control, fixed construction, or stop/pivot reason",
        "- after scout, continue through `prepare-branch`, `debug-branch`, and `run-branch`; make strategic continue/stop/report decisions only after `run-branch` records a result",
