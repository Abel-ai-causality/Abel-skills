...
def build_context_guide_markdown(
    *,
    ...
    lines = [
        ...
        "- `evidence_intent`: candidate, diagnostic, or draft; reserve control for explicit user/protocol control studies",
        ...
        "",
        "## Disposable Search",
        "- temporary scripts, scored screens, and compact summaries may live in the session `scratch/` directory",
        ...
        "- expect the first-look scout to take roughly 5 minutes as a quick, broad shortlist builder for candidate directions and coarse variants; its job is to produce candidates for recorded branch validation, not to keep optimizing in scratch",
        "- keep each ScoutRun about 10,000 candidates or fewer",
        "- batch scratch scoring that materially chooses a recorded candidate should use `ScoutRun(name, scratch).run(candidates, scorer)` so partial rows are preserved",
        "- write ScoutRun artifacts directly in the session `scratch/` root with a unique scout name; do not use nested scratch directories for ScoutRun outputs",
        "- if ScoutRun stops after streaming partial rows, treat the available rows as the current scout output",
        "- once usable rows exist, promote meaningful recorded branch work and account for material selection width",
        "- after a recorded branch runs, prefer another meaningful candidate from existing evidence over more scratch search around the same branch",
        "- a strong lead belongs in the ledger; it is not a place to keep searching locally",
        "- target-history-only and buy-and-hold rows are reference metrics, not ordinary recorded candidates; record target-history-only only when no usable non-target data supports the search or the user explicitly asks for that shape",
        "- a prepared scout branch can be prepare-only; do not run a flat/no-signal materialization round",
        "- IC/correlation/feature-importance tables are diagnostics; a first-look scout should rank scored candidate-shaped variants when broader comparison remains useful",
        "- scratch/probe outputs are decision-support material, not validation evidence",
        ...
        "- `run-branch` is for meaningful strategy candidates: if validation supports the branch, it could be selected or reported to the user",
        "",
        "## Audit Checklist",
