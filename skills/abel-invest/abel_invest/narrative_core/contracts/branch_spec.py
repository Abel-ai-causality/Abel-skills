...
def build_context_guide_markdown(
    *,
    ...
    lines = [
        ...
        "- expect the first-look scout to fit about 120 seconds and choose a direction before recorded work, not become open-ended private search",
        "- make scout scripts bounded and resumable: run `--dry-run` first with a free-form family budget declaration, treat declared budget seconds as search-space disclosure rather than trusted timing, reduce over-budget or slow-candidate families before execution, stream completed candidates to `scratch/*.results.jsonl`, keep `scratch/*.state.json` and `scratch/*.summary.json` current, and use `--resume` after interruption instead of restarting",
        "- treat heavy tree/GBDT/HGBT or large walk-forward model families as evidence-triggered second-stage probes, not first-look defaults",
        "- the session scout budget is first-look plus at most one narrow refinement tied to one recorded blocker or top family; after that the same phase has at most one non-control continuation branch before best-strategy/report",
        "- a near-pass, graph expansion, or model-capacity idea can justify that one continuation slot, but should not start another scratch grid or chain of fixed graph/model/mechanism probes unless the user asks for a new phase",
        "- use the minimal `ScoutRun` pattern in `experiment-loop.md`; do not inspect `scout_runtime.py` or probe helper signatures unless the helper import fails or you are debugging the helper",
        "- keep scout stdout compact: dry-run summary, periodic progress, final top results, and artifact paths only; full ranked tables stay on disk",
