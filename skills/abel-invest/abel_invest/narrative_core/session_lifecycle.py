...
def render_data_led_start_lines(session: Path) -> list[str]:
    ...
    return [
        ...
        "use a compact scored scout to choose, not just describe: rank plausible target, graph, and construction shapes before broad recorded work",
        "in scratch scripts, import the packaged helper with `from abel_invest.narrative_core.scout_runtime import ScoutRun`",
        "when a scout or scratch batch scores multiple candidate variants, use ScoutRun(name, scratch).run(candidates, scorer, sort_key=...) so compact result rows stream to disk, resumable artifacts survive interruptions, and the top summary is ranked by your explicit strategy criterion",
        "ScoutRun is a stability helper for batch scout execution, not a separate research phase or strategy constraint",
        "promote the strongest ranked shapes into recorded branch work and account for any selection width that materially chose the submitted candidate",
