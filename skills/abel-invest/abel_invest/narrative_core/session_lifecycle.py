...
def render_data_led_start_lines(session: Path) -> list[str]:
    ...
    return [
        ...
        "when scratch work evaluates a set of candidate variants to choose what to validate, import `from abel_invest.narrative_core.scout_runtime import ScoutRun` and use ScoutRun(name, scratch).run(candidates, scorer) so result rows stream to disk while the scout runs",
        "if the helper stops after streaming partial rows, use the available rows as the current scout output and decide what to validate",
