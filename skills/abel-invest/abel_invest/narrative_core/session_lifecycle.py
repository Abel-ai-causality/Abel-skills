...
def render_data_led_start_lines(session: Path) -> list[str]:
    ...
    return [
        ...
        f"after prepare-branch, temporary scripts or summaries may live in {scratch_dir} or an equivalent heredoc/notebook/query cell; they are normal research workbench material, not validation evidence",
        ...
        "expect the first-look scout to be a few-minute scratch search: score plausible target, graph, and construction shapes, then rank what looks worth formal validation before broad recorded work",
        "when scratch work evaluates a set of candidate variants to choose what to validate, import `from abel_invest.narrative_core.scout_runtime import ScoutRun` and use ScoutRun(name, scratch).run(candidates, scorer) so result rows stream to disk and the run can timeout/resume safely",
        "promote the strongest shapes into recorded branch work and account for material selection width",
        ...
        "use target-only as baseline, seed, ablation, or competitor; simple hand-written rules are diagnostics, controls, ablations, or refinements around empirical construction, not the default main lane",
