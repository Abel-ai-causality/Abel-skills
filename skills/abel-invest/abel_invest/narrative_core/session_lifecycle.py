...
def render_data_led_start_lines(session: Path) -> list[str]:
    ...
    return [
        ...
        "use a compact scored scout as quick direction finding: score plausible target, graph, and construction shapes, then rank what looks worth formal validation before broad recorded work",
        "when graph leads rank as a group, consider whether that group supports vote, average, ensemble, confidence, or sizing constructions",
        "when a scratch script scores or ranks multiple candidate variants to choose what to validate, import the packaged helper with `from abel_invest.narrative_core.scout_runtime import ScoutRun`",
        "use ScoutRun(name, scratch).run(candidates, scorer, sort_key=...) so results stream to disk and resumable artifacts survive interruptions",
