...
def render_data_led_start_lines(session: Path) -> list[str]:
    ...
    return [
        ...
        "make scout scripts bounded and resumable: run --dry-run first with a free-form family budget declaration, treat declared budget seconds as advisory search-shape disclosure rather than trusted timing or an execution gate, stream results to scratch/*.results.jsonl, keep scratch/*.state.json and scratch/*.summary.json current, and resume interrupted work instead of restarting",
