...
def render_data_led_start_lines(session: Path) -> list[str]:
    ...
    return [
        ...
        f"after prepare-branch, temporary scripts or summaries may live in {scratch_dir} or an equivalent heredoc/notebook/query cell; they are decision-support material, not validation evidence",
        ...
        "batch scratch scoring that materially chooses a recorded candidate should use the scout runtime boundary described in experiment-loop.md and write ScoutRun artifacts directly in the session scratch root",
        "if ScoutRun stops after partial rows, treat the available rows as the current scout output",
        "once usable rows exist, promote meaningful recorded branch work and account for material selection width",
        "after a recorded branch fails, prefer another meaningful branch from existing evidence over more scratch search around the same branch",
        ...
        "let data decide how to express graph-derived information; do not treat any construction pattern as a fixed route",
        "run recorded strategy candidates only after data/cache are prepared and the candidate shape is understood",
