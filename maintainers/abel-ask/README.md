...
in the selected profile. The rendered `dist/local/abel-ask/scripts/narrative_cap_probe.py`
...
```bash
python3 maintainers/abel-ask/smoke_narrative_cap_probe.py
```

For JSON output or a different rendered skill root:

```bash
python3 maintainers/abel-ask/smoke_narrative_cap_probe.py --json
python3 maintainers/abel-ask/smoke_narrative_cap_probe.py --skill-root skills/abel-ask
```
...
- The publish flow reads from collection source, so render `skills/abel-ask/` from `maintainers/skills/` first if endpoint-facing text changed.
